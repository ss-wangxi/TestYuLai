...
public class NetworkUtils {
    ...
    public static String httpGetData(String url, List<NameValuePair> params, int timeout, String cookie) {
        final StringBuilder sbFinalUrl = new StringBuilder(url);
        if (params != null && params.size() > 0) {
            final int size = params.size();
            for (int p = 0; p != size; ++p) {
                final NameValuePair param = params.get(p);
                sbFinalUrl.append(p == 0 && !url.contains("?") ? "?" : "&");
                sbFinalUrl.append(param.getName());
                sbFinalUrl.append("=");
                sbFinalUrl.append(param.getValue());
            }
        }
        return httpGetData(sbFinalUrl.toString(), timeout, cookie);
    }
